...
from flask_swagger_ui import get_swaggerui_blueprintcls
...
def add_candidate():
    ...
    cursor = db.cursor()
    sql = "INSERT INTO candidate (name, age, DateOfBirth, qualification, assets_value, party, address, district, state, country) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"
    val = (name, age, DateOfBirth, qualification, assets_value, party, address, district, state, country)
    cursor.execute(sql, val)
    db.commit()
    cursor.close()

    return jsonify({
                    "message": "Candidate record inserted successfully",
                    "user":jwt_login_user,
                    })
...
if __name__ == '__main__':
    #app.run()
    app.run(host='127.0.0.1', port=5002) # Change the port as needed
    #app.run(debug=True)
